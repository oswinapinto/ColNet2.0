# code reference - https://github.com/alan-turing-institute/SemAIDA/tree/master/AAAI19/exp_T2D
...
def ordered_cells2synthetic_columns(col_cells, synthetic_column_size):
    ...
    if len(col_cells) >= synthetic_column_size:
        ...
        cell_unit = col_cells + ['NaN'] * \
            (len(col_cells) - synthetic_column_size)
        cell_units.append(cell_unit)
    return cell_units
...
def generate_synthetic_columns(entities, synthetic_column_size):
    ...
    if len(entities) >= synthetic_column_size:
        for i, ent in enumerate(entities):
            unit = random.sample(
                entities[0:i] + entities[(i + 1):], synthetic_column_size - 1)
            unit.append(ent)
...
class SyntheticColumnCNN(tf.Module):
    """
    ...
    def __init__(self, sequence_length, num_classes, embedding_size, channel_num, filter_sizes, num_filters):
        ...
        self.input_x = tf.keras.layers.Input(shape=(
            sequence_length, embedding_size, channel_num), dtype=tf.float32, name="input_x")
        self.input_y = tf.keras.layers.Input(
            shape=(num_classes,), dtype=tf.float32, name="input_y")
        self.dropout_keep_prob = tf.keras.layers.Input(
            shape=(), dtype=tf.float32, name="dropout_keep_prob")
        ...
        for i, filter_size in enumerate(filter_sizes):
            with tf.name_scope("conv-max_pool-%s" % filter_size):
                ...
                Conv_W = tf.Variable(tf.truncated_normal(
                    filter_shape, stddev=0.1), name="Conv_W")
                Conv_b = tf.Variable(tf.constant(
                    0.1, shape=[num_filters]), name="Conv_b")
                conv = tf.nn.conv2d(
                    self.input_x,
                    Conv_W,
                    strides=[1, 1, 1, 1],
        ...
        with tf.name_scope("output"):
            FC_W = tf.Variable(tf.initializers.glorot_normal()(
                shape=[num_filters_total, num_classes]), name="FC_W")
            FC_b = tf.Variable(tf.constant(
                0.1, shape=[num_classes]), name="FC_b")
            self.scores = tf.nn.xw_plus_b(
                self.h_drop, FC_W, FC_b, name="scores")
            self.probabilities = tf.nn.softmax(
                self.scores, axis=-1, name='probabilities')
            self.predictions = tf.argmax(
                self.scores, axis=1, name="predictions")
        ...
        with tf.name_scope("accuracy"):
            correct_predictions = tf.equal(
                self.predictions, tf.argmax(self.input_y, axis=1))
